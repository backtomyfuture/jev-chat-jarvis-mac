...
微信弹出一条消息 → 悬浮窗立刻告诉你**这句话的真实意图**、**时效压力**、**风险几级**、**该怎么回**。

**纯只读、零封号风险**——不注入、不 hook、不解密数据库，只是「看屏幕 + 云端智能研判与生成」。全云端轻量化运行，无需下载任何庞大本地模型。
...
- **诉求 + 时效 + 建议 + 独立风险**：全面升级多维研判体系——精准识别对方真实诉求、时效压力与我方得体应对策略；严格基于客观证据识别隐私/合规/资金/承诺风险，人身安全与暴力恐吓威胁最高优先级拦截并报警建议（催促绝不误报为危险）；云端 Jev 8 类意图 **86.4%** 回归基准。
- **候选回复与策略避坑联动**：内置 10 种话术并发生成，生成层深度联动研判层的「应对建议」与「避坑防范指南」→ 先上屏 → 云端 Jev 精准排序；换话术立刻重新生成。
- **极速轻量 · 全云端架构**：消息一出现判断与生成同时起跑，端到端 ~1.5–2 s；**不下载任何数 GB 的本地模型**，零 GPU / 显存占用，轻巧省电。
- **前台隐私守卫**：微信切出前台时立即清空聊天气泡并退至后台隐藏，绝不将私人聊天残留在屏幕上。
- **YOLO 检测框**（可选，`JEV_BOXES=1` 启动即开、菜单栏可切）：OCR 命中的消息实时框在微信窗口上，对方/我分色 + 置信度。

## 面板读法

面板使用 macOS 原生浅色磨砂材质：顶部是当前聊天、会话选择、分析状态、正在处理的消息与上下文；中间以时间线气泡和卡片展示「要你: 诉求 · 时效: 截止 · 建议: 应对建议」，或在触发风险时醒目提示「⚠️ 风险类别 · 建议: 应对建议」；底部按话术分组展示候选回复。当前风险圆点会轻微呼吸提示。每条候选左侧是云端排序概率，右侧是「复制」和「填入」；候选行会随完整文字自动增高，不截断内容，发送始终由用户在微信里手动完成。
微信切出前台时面板自动隐藏并清空对话，守卫多任务与投屏时的屏幕隐私。
...
**仅支持 Apple Silicon（M 系列）Mac**，macOS 13+。不支持 Intel Mac，也不要通过 Rosetta 运行；启动时会检测并以中文提示原因。
...
uv run python src/judge.py "这个需求你今天跟一下"  # 单条消息出多维研判（诉求/时效/建议/风险）
uv run python src/judge_zh_test.py               # 22 条中文意图回归
uv run python src/generate.py --check            # 生成层凭据解析
uv run python -B -m unittest discover -s tests   # 发出消息/异步结果/多维研判回归（合成 OCR，不读屏）
uv run python probe/bootstrap_regression.py      # 两种启动入口的离线回归；不联网、不实际安装
```

## 配置

本项目**完全走云端 API，不下载也不运行任何本地模型**（告别数 GB 的权重下载等待与系统显存压力）。
两层配置均通过环境变量或配置文件完成，生成层打包版内置免费通道，不配也能直接出候选，详细数据流向见 [PRIVACY.md](PRIVACY.md)。全部配置保存在一个 env 文件中（**不提供第二种格式**）：
...
- 密钥掩码显示；窗口仅读取所编辑文件中的值，不把环境变量、项目 `.env` 或内置共享密钥复制进用户文件。各配置页顶部突出显示本次启动正在使用的密钥与服务来源；生成页同时标明当前启用的服务，优先级保留在窗口下方。
- 环境变量优先于用户 env，用户 env 优先于项目 `.env`；生成层 OpenAI 组优先于 Anthropic 组，均未配置才使用内置共享密钥。清空当前文件的密钥不会禁用其他来源中的密钥。由终端或启动器导出的值也显示为「环境变量」。
- API 格式由密钥组决定：`OPENAI_*` 使用 OpenAI 格式，`ANTHROPIC_*` 使用 Anthropic 格式；自定义地址不需要包含服务名称。Jev 地址带不带末尾 `/v1` 都行，与手动配置共用同一条拼接规则。
...
# 判断层：TypeSafe Jev（结构化云端研判，输出诉求/时效/建议/独立风险）
export TYPESAFE_API_KEY="你的TypeSafe_key"
export TYPESAFE_BASE_URL="https://api.typesafe.ai"
export TYPESAFE_MODEL="jev-latest"

# 生成层：任意 OpenAI 兼容端点（如 DeepSeek、智谱、StepFun、MiniMax 等）
...
- **凭据解析以 key 为准**：提供 key 的来源同时决定端点和模型。实测可用：DeepSeek `deepseek-chat`（最快）；StepFun `step-3.7-flash`；智谱 `glm-4-flash`（换 `ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL`/`ANTHROPIC_MODEL`，两组都填 OpenAI 组优先）
...
- 自查凭据（不打印完整 key）：`uv run python src/generate.py --check`、`uv run python src/judge.py`

## 磁盘占用与清理

全云端运行架构，**零本地大模型权重占用**（无需下载 3.8 GB 的模型权重）：

| 内容 | 位置 | 大小 | 说明 |
|---|---|---|---|
| Python 运行环境（venv） | `~/Library/Application Support/jev-jarvis/venv` | ~0.7 GB | 基础 Python 依赖运行库；删除 .app 不会连带删它，需手动删 |
| 配置文件 | `~/.config/jev-jarvis/env` | < 1 KB | 保存你的云端 API 凭据与偏好设置 |

## 已知限制

- 悬浮窗只在微信位于前台时显示和分析；切到浏览器等其他应用会立即隐藏并清空敏感聊天气泡，返回微信后强制重新读取当前聊天，保护屏幕隐私
...
- 启动后第一条判断因网络首连建立连接池可能多耗时约 0.2s，属于正常现象；不对劲先看日志（分阶段耗时、**不含消息正文**，可放心贴 issue）：`tail -40 ~/Library/Logs/jev-jarvis.log`
...
- 架构一句话：微信在前台时，进程内抓其窗口 → Vision OCR（本地只读聊天区）→ 云端 Jev 出诉求/时效/建议/独立风险 → 云端 LLM 联动策略并发出候选 → 云端 Jev 排序 → 悬浮窗 NSPanel。底层仍按窗口 ID 抓取而不是全屏截图，悬浮窗不污染 OCR
...
**隐私与数据流向**详见 [PRIVACY.md](PRIVACY.md)：全云端轻量化运行，图片与抓图绝不上传，聊天文字仅发往模型服务商（推荐自配 API key 直连）；前台切出自动清空遮蔽，全面守卫屏幕隐私。
