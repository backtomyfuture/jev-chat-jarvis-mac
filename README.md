...
微信弹出一条消息 → 悬浮窗立刻告诉你**这句话的真实诉求**、**时效压力**、**该怎么回**。

**纯只读、零封号风险**——不注入、不 hook、不解密数据库，只是「看屏幕 + 云端智能研判与生成」。全云端轻量化运行，无需下载任何庞大本地模型。

![演示：微信群消息进来 → 面板给出诉求、时效、建议与候选回复 → 点「填入」直接进微信输入框](docs/demo.gif)
...
- **诉求 + 时效 + 建议**：个人聊天研判——识别对方诉求、时效压力，并给出得体应对建议。不做安全、合规、资金或人身威胁分类；云端 Jev 8 类意图 **86.4%** 回归基准。
- **候选回复与建议联动**：内置 10 种话术并发生成，生成层带上研判层的「应对建议」→ 先上屏 → 云端 Jev 精准排序；换话术立刻重新生成。
...
面板使用 macOS 原生浅色磨砂材质：顶部是当前聊天、会话选择、分析状态、正在处理的消息与上下文；中间以时间线气泡和卡片展示「要你: 诉求 · 时效: 截止 · 建议: 应对建议」；底部按话术分组展示候选回复。每条候选左侧是云端排序概率，右侧是「复制」和「填入」；候选行会随完整文字自动增高，不截断内容，发送始终由用户在微信里手动完成。
...
uv run python src/judge.py "这个需求你今天跟一下"  # 单条消息出多维研判（诉求/时效/建议）
...
# 判断层：TypeSafe Jev（结构化云端研判，输出诉求/时效/建议）
...
- 架构一句话：微信在前台时，进程内抓其窗口 → Vision OCR（本地只读聊天区）→ 云端 Jev 出诉求/时效/建议 → 云端 LLM 联动建议并发出候选 → 云端 Jev 排序 → 悬浮窗 NSPanel。底层仍按窗口 ID 抓取而不是全屏截图，悬浮窗不污染 OCR
